    ...
    def forward(self, x):
        ...
        if self.num_segments is not None and self.fusion is not None:
            ...
            for segment in range(self.num_segments):
                ...
                weighted_value[left_pos:right_pos] = func.normalize(weighted_value[left_pos:right_pos].clone(), p=2,
                                                                    dim=1)
                weighted_local_value = func.normalize(weighted_local_value, p=2, dim=1)
                if self.fusion == "add":
